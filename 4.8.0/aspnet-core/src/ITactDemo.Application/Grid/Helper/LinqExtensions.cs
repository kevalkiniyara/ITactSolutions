...
namespace ITactDemo.Grid.Helper
{
    public static class LinqExtensions
    {
            ...
            string column, object value, WhereOperation operation)
        {
            ...
            object conType;
            if (memberAccess.Type.GetTypeInfo().IsGenericType && memberAccess.Type.GetGenericTypeDefinition() == typeof(Nullable<>))
                conType = Convert.ChangeType(value, Nullable.GetUnderlyingType(memberAccess.Type));
            else
            {
                if (memberAccess.Type.GetTypeInfo().IsEnum)//Handle enum type
                    conType = Enum.Parse(memberAccess.Type, value.ToString());
                else
