...
var JQ_sopt_bool = JSON.parse('{"sopt": ["eq","ne"], "value": ":All;True:True;False:False"}');
var JQ_sopt_int = JSON.parse('{"sopt": ["eq","ne","le","lt","gt","ge"]}');
var JQ_sopt_date = JSON.parse('{"sopt": ["eq","ge","le"]}');
var JQ_sopt_equal = JSON.parse('{"sopt":["eq"]}');
...
(function ($) {
    ...
    jQuery.extend(jQuery.jgrid.defaults, {
        ...
        ajaxRowOptions: { //!!!important Required for inline editing to work
            type: "POST",
    ...
    jQuery(window).on('resize.jqGrid', function () {
        jQuery('[id^=gbox_]').each(function () {
            var grid_selector = '#' + jQuery(this).attr('id').substring(5);
            jQuery(grid_selector).jqGrid('setGridWidth', jQuery(grid_selector).closest(".jqgrid").width());
        });
    });
})();
...
(function ($) {
    ...
    app.UserNotificationHelper = (function () {
        ...
        return function () {
            ...
            /* Example:
            abp.notifications.messageFormatters['EZTBM.MyNotificationDataType'] = function(userNotification) {
            ...
            */
            ...
            function getUrl(userNotification) {
                switch (userNotification.notification.notificationName) {
                    ...
                    case 'App.ApproveOrRejectMessage':
                        return '/App/BudgetingAndForecasting/CreateOrUpdateBudgetSheet/' + userNotification.notification.data.properties.budgetPlanCCId;
...
(function () {
    ...
    var appLocalizationSource = abp.localization.getSource('EZTBM');
    ...
    app.createAllocationDateRangePickerOptions = function () {
        var options = {
            locale: {
                format: 'L',
                applyLabel: app.localize('Apply'),
            ...
            min: moment().subtract(3, 'year').startOf('year'),
            minDate: moment().subtract(3, 'year').startOf('year'),
            max: moment().add('year', 1).endOf('year'),
            maxDate: moment().add('year', 1).endOf('year'),
            ranges: {}
        };
        options.ranges[app.localize('Current Month')] = [moment().startOf('month'), moment().endOf('month')];
        options.ranges[app.localize('Current Quarter')] = [moment().startOf('quarter'), moment().endOf('quarter')];
        options.ranges[app.localize('Current Year')] = [moment().startOf('year'), moment().endOf('year')];
        options.ranges[app.localize('Previous Month')] = [moment().subtract(1, 'month').startOf('month'), moment().subtract(1, 'month').endOf('month')];
        options.ranges[app.localize('Previous Quarter')] = [moment().subtract(1, 'quarter').startOf('quarter'), moment().subtract(1, 'quarter').endOf('quarter')];
        options.ranges[app.localize('Previous 2 Quarters')] = [moment().subtract(2, 'quarter').startOf('quarter'), moment().subtract(1, 'quarter').endOf('quarter')];
        options.ranges[app.localize('Previous 3 Quarters')] = [moment().subtract(3, 'quarter').startOf('quarter'), moment().subtract(1, 'quarter').endOf('quarter')];
        options.ranges[app.localize('Previous Year')] = [moment().subtract(1, 'year').startOf('year'), moment().subtract(1, 'year').endOf('year')];
        options.ranges[app.localize('Previous 2 Years')] = [moment().subtract(2, 'year').startOf('year'), moment().subtract(1, 'year').endOf('year')];
        options.ranges[app.localize('Previous 3 Years')] = [moment().subtract(3, 'year').startOf('year'), moment().subtract(1, 'year').endOf('year')];
        options.ranges[app.localize('Next Month')] = [moment().add('month', 1).startOf('month'), moment().add('month', 1).endOf('month')];
        options.ranges[app.localize('Next Quarter')] = [moment().add(1, 'quarter').startOf('quarter'), moment().add(1, 'quarter').endOf('quarter')];
        options.ranges[app.localize('Next 2 Quarters')] = [moment().add(1, 'quarter').startOf('quarter'), moment().add(2, 'quarter').endOf('quarter')];
        options.ranges[app.localize('Next 3 Quarters')] = [moment().add(1, 'quarter').startOf('quarter'), moment().add(3, 'quarter').endOf('quarter')];
        options.ranges[app.localize('Next Year')] = [moment().add('year', 1).startOf('year'), moment().add('year', 1).endOf('year')];
        options.ranges[app.localize('Current+Next Year')] = [moment().startOf('year'), moment().add('year', 1).endOf('year')];

        return options;
    };

    app.createCostCategoryAllocationDateRangePickerOptions = function () {
        var options = {
            locale: {
                format: 'L',
                applyLabel: app.localize('Apply'),
    ...
    app.objToJQgridFilterString = function (obj) {
        
        var str = '';
        $.each(Object.keys(obj).sort(), function (i, p) {
...
(function ($) {
    ...
    $.fn.serializeFormDataToObject = function () {
        //serialize to array
        var data = $(this).serializeArray();
